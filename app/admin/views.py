...
# 上下文处理器(它可以封装我们的全局变量,然后把我们的全局变量展现到我们的模板里面)
...
@admin.route('/movie/add/upload/', methods=['POST'])
@admin_login_req
@admin_auth
def upload_part():  # 接收前端上传的一个分片
    ...
    upload_file.save('./app/static/upload/%s' % filename)  # 保存分片到本地
    return render_template('admin/admin.html')


@admin.route('/movie/add/merge/', methods=['GET'])
@admin_login_req
@admin_auth
def upload_success():  # 按序读出分片内容，并写入新文件
    ...
    chunk = 0  # 分片序号
    with open('./app/static/upload/%s%s' % (task, ext), 'wb') as target_file:  # 创建新文件
        while True:
            try:
...
def oplog_list(page=None):
    ...
    page_data = Oplog.query.join(
        Admin).filter(Admin.id == Oplog.admin_id,
...
def adminloginlog_list(page=None):
    ...
    page_data = Adminlog.query.join(
        Admin).filter(Admin.id == Adminlog.admin_id,
...
def userloginlog_list(page=None):
    ...
    page_data = Userlog.query.join(
        User).filter(User.id == Userlog.user_id,
...
def role_list(page=None):
    if page is None:
        page = 1
    page_data = Role.query.order_by(Role.addtime.desc()
                                    ).paginate(page=page, per_page=10)
